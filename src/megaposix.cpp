...
Using cURL for HTTP I/O, GNU Readline for console I/O and FreeImage for
thumbnail creation
...
CurlHttpIO::CurlHttpIO() {
  ...
  contenttypebinary =
      curl_slist_append(NULL, "Content-Type: application/octet-stream");
  contenttypebinary = curl_slist_append(contenttypebinary, "Expect:");
}

CurlHttpIO::~CurlHttpIO() { curl_global_cleanup(); }
...
int CurlHttpIO::doio() {
  ...
  while ((msg = curl_multi_info_read(curlm, &dummy))) {
    ...
    if (curl_easy_getinfo(msg->easy_handle, CURLINFO_PRIVATE, (char **)&req) ==
            CURLE_OK &&
        req) {
      req->httpio = NULL;
      ...
      if (msg->msg == CURLMSG_DONE) {
        ...
        if (debug) {
          ...
          if (req->binary)
            cout << "[received " << req->in.size() << " bytes of raw data]"
                 << endl;
          else
...
size_t CurlHttpIO::write_data(void *ptr, size_t size, size_t nmemb,
                              void *target) {
  size *= nmemb;
...
FileAccess *DemoApp::newfile() { return new PosixFileAccess(); }

PosixFileAccess::PosixFileAccess() { fd = -1; }
...
int PosixFileAccess::fread(string *dst, unsigned len, unsigned pad,
                           m_off_t pos) {
  dst->resize(len + pad);
...
int PosixFileAccess::fopen(const char *f, int read, int write) {
  if ((fd = open(f, write ? (read ? O_RDWR : O_WRONLY | O_CREAT) : O_RDONLY,
                 0500)) >= 0) {
    struct stat statbuf;
...
int unlink_file(const char *name) { return !unlink(name); }

int change_dir(const char *name) { return !chdir(name); }

int globenqueue(const char *path, const char *newname, handle target,
  ...
  for (int i = 0; i < (int)globbuf.gl_pathc; i++) {
    if (!stat(globbuf.gl_pathv[i], &statbuf) && statbuf.st_mode & S_IFREG) {
      for (filename = strchr(globbuf.gl_pathv[i], 0);
           filename > globbuf.gl_pathv[i] && *filename != ':' &&
           *filename != '/';
           filename--)
        ;
      ...
      if (*filename) {
        client->putq.push_back(
            new AppFilePut(filename, target, targetuser, newname));
        total++;
      }
    }
  }
