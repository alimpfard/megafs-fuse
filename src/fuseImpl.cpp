...
int hello_truncate(const char *a, off_t o) { return megaFuse->truncate(a, o); }
...
int hello_setxattr(const char *path, const char *name, const char *value,
                   size_t size, int flags) {
  std::lock_guard<std::mutex> lock(api_mutex);
  return megaFuse->setxattr(path, name, value, size, flags);
}
int hello_getxattr(const char *path, const char *name, char *value,
                   size_t size) {
