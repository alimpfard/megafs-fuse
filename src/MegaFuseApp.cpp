...
void MegaFuseApp::nodes_updated(Node **n, int c) {
  ...
  for (int i = 0; i < c; i++) {
    ...
    if (!removed && currentCache != model->cacheManager.end() &&
        fullpath != currentCache->first) {
...
void MegaFuseApp::topen_result(int td, error e) {
  printf("topen!\n");
  client->tclose(td);
  for (auto it = model->cacheManager.begin(); it != model->cacheManager.end();
       ++it)
    if (it->second.td == td)
      it->second.td = -1;
...
void MegaFuseApp::topen_result(int td, string *filename, const char *fa,
                               int pfa) {
  printf("topen result open\n");
  ...
  for (auto it = model->cacheManager.begin(); it != model->cacheManager.end();
    ...
    if (it->second.td == td) {
      ...
      if (it->second.status == file_cache_row::INVALID) {
        ...
        it->second.availableChunks.resize(
            CacheManager::numChunks(it->second.size), false);
      }
    }
  //
  client->dlopen(td, tmp.c_str());
  printf("file: %s in state DOWNLOADING\n", remotename.c_str());
...
// download completed
void MegaFuseApp::transfer_complete(int td, chunkmac_map *macs,
                                    const char *fn) {
  printf("\ndownload complete\n\n");
  ...
  if (missingOffset < 0) {
    ...
    int neededBytes = -1;
    for (unsigned int i = startBlock; i < it->second.availableChunks.size();
         i++) {
      if (it->second.availableChunks[i]) {
        // workaround 2, download a bit more because the client is not always
        // updated at the block boundary
        neededBytes =
            ChunkedHash::SEGSIZE + CacheManager::blockOffset(i) - startOffset;
        break;
      }
    }
    ...
    if (startOffset + neededBytes > it->second.size)
      neededBytes = -1;
    printf("------download resume, missing %d bytes starting from block %d\n",
...
void MegaFuseApp::transfer_complete(int td, handle ulhandle,
                                    const byte *ultoken, const byte *filekey,
                                    SymmCipher *key) {
  ...
  "'s inbox..." << endl; client->putnodes(putf->targetuser.c_str(),newnode,1);
  } else*/
...
void MegaFuseApp::transfer_update(int td, m_off_t bytes, m_off_t size,
                                  dstime starttime) {
  std::string remotename = "";
  if (model->cacheManager.findByTransfer(td, file_cache_row::UPLOADING) !=
      model->cacheManager.end()) {
    cout << '\r' << "UPLOAD TD " << td << ": Update: " << bytes / 1024
         << " KB of " << size / 1024 << " KB, "
         << bytes * 10 / (1024 * (client->httpio->ds - starttime) + 1)
         << " KB/s";
    fflush(stdout);
    return;
  }
  ...
  it->second.available_bytes = ChunkedHash::chunkfloor(
      ChunkedHash::chunkfloor(it->second.startOffset) + bytes);

  int endChunk = CacheManager::numChunks(
      ChunkedHash::chunkfloor(it->second.available_bytes));
    ...
    try {
      if (!it->second.availableChunks[i]) {
        ...
        printf("block %d/%d available\n", i,
               (int)it->second.availableChunks.size());
      }

    } catch (...) {
      printf("block %d fucked up\n", i);
      fflush(stdout);
      abort();
    }
  }
  {
    ...
    // ChunkedHash::chunkfloor(ChunkedHash::chunkfloor(it->second.startOffset) +
    // bytes) <<std::endl;
    static time_t last_update = time(NULL);
    if (last_update < time(NULL)) {
      std::cout << rigaDownload << size / 1024 / 1024 << " MB, "
                << bytes * 10 / (1024 * (client->httpio->ds - starttime) + 1)
                << " KB/s, available_b/size " << it->second.available_bytes
                << "/" << it->second.size << " "
                << (it->second.startOffset + bytes) << "/" << size
                << "            ";
  ...
  if (it->second.startOffset && it->second.available_bytes >= it->second.size) {
    printf("workaround 1\n"); // have to call manually if the download didn't
                              // start at 0
    transfer_complete(td, NULL, NULL);
  } else if (endChunk > startChunk &&
             endChunk < it->second.availableChunks.size() &&
             it->second.availableChunks[endChunk]) {
