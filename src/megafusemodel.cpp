...
MegaFuseModel::MegaFuseModel(EventsHandler &eh, std::mutex &em)
    : engine_mutex(em), eh(eh), callbacksHandler(this) {}
...
void MegaFuseModel::check_cache() {
  ...
  for (auto it = cacheManager.begin(); it != cacheManager.end(); ++it)
    if (it->second.n_clients == 0 &&
      ...
      printf("removing the file '%s' from cache\n", it->first.c_str());
      if (it->second.status ==
          file_cache_row::DOWNLOADING) // UPLOADING FILES IGNORED
...
MegaApp *MegaFuseModel::getCallbacksHandler() { return &callbacksHandler; }
...
int MegaFuseModel::open(const char *p, struct fuse_file_info *fi) {
  ...
  bool oldCache =
      it != cacheManager.end() && n && it->second.last_modified < n->mtime;
...
int MegaFuseModel::write(const char *path, const char *buf, size_t size,
  ...
  if (it->second.size < newsize) {
    ...
    it->second.availableChunks.resize(CacheManager::numChunks(it->second.size),
                                      false);
  }
  return s;
}
// 0 is success
int MegaFuseModel::makeAvailableForRead(const char *path, off_t offset,
                                        size_t size) {
  std::unique_lock<std::mutex> engine(engine_mutex);
  ...
  case file_cache_row::DOWNLOADING:
    if (it->second.chunksAvailable(offset, size))
      return 0;
    if (((offset + size) - (it->second.available_bytes)) < 2 * 1024 * 1024 &&
  ...
  if (startDownload) {
    ...
    if (open_result.result < 0) {
      printf("error while waiting for topen_result, file: %s\n",
...
int MegaFuseModel::read(const char *path, char *buf, size_t size, off_t offset,
                        struct fuse_file_info *fi) {
  std::unique_lock<std::mutex> engine(engine_mutex);

  int fd = ::open(cacheManager[path].localname.c_str(), O_RDONLY);
  if (fd < 0) {
    printf("open failed in reading of file '%s'\n",
