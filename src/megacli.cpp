...
static byte signuppwchallenge[SymmCipher::KEYLENGTH],
    signupencryptedmasterkey[SymmCipher::KEYLENGTH];

// attempt to create a size*size JPEG thumbnail using FreeImage
// thumbnail specs:
// - largest square crop at the center (landscape) or at 1/6 of the height above
// center (portrait)
...
void createthumbnail(const char *filename, unsigned size, string *result) {
  ...
  if (fif == FIF_JPEG) {
    ...
    if (!(dib = FreeImage_Load(fif, filename,
  ...
  if ((tdib = FreeImage_Rescale(dib, w, h, FILTER_BILINEAR))) {
    ...
    if ((tdib = FreeImage_Copy(dib, (w - size) / 2, (h - size) / 3,
                               size + (w - size) / 2, size + (h - size) / 3))) {
      FreeImage_Unload(dib);
      ...
      if ((hmem = FreeImage_OpenMemory())) {
        if (FreeImage_SaveToMemory(FIF_JPEG, dib, hmem,
                                   JPEG_BASELINE | JPEG_OPTIMIZE | 85)) {
          BYTE *tdata;
          DWORD tlen;
...
void AppFilePut::start() {
  cout << "Sending " << filename << " (attempt #" << ++failcount << ")..."
       << endl;
  ...
  if (td < 0) {
    ...
    cout << filename << ": Send failed (" << errorstring((error)td)
         << "), retrying in " << bt.retryin(client->httpio->ds) * 100
         << " ms..." << endl;
  } else {
    string thumbnail;

    // (thumbnail creation should be performed in subthreads to keep the app
    // nonblocking) to guard against file overwrite race conditions, production
    // applications should use the same file handle for uploading and creating
    // the thumbnail
    createthumbnail(filename.c_str(), 120, &thumbnail);

    if (thumbnail.size()) {
      cout << "Image detected and thumbnail extracted, size "
           << thumbnail.size() << " bytes" << endl;
      ...
      client->putfa(&client->ft[td].key, uh, THUMBNAIL120X120,
                    (const byte *)thumbnail.data(), thumbnail.size());
    }
  }
}
...
void AppFileGet::start() {
  ...
  if ((n = client->nodebyhandle(nodehandle))) {
    cout << "Fetching " << n->displayname() << " (attempt #" << failcount
         << ")..." << endl;
  ...
  if (!n) {
    bt.backoff(client->httpio->ds);
    cout << "retrying in " << bt.retryin(client->httpio->ds) * 100 << " ms..."
         << endl;
  }
}
...
void DemoApp::topen_result(int td, error e) {
  cout << "TD " << td << ": Failed to open file (" << errorstring(e) << ")"
       << endl;
...
void DemoApp::topen_result(int td, string *filename, const char *fa, int pfa) {
  cout << "TD " << td << ": File opened successfully, filename: " << *filename
       << endl;

  if (fa && debug)
    cout << "File has attributes: " << fa << " / " << pfa << endl;
...
void DemoApp::transfer_update(int td, m_off_t bytes, m_off_t size,
                              dstime starttime) {
  cout << "TD " << td << ": Update: " << bytes / 1024 << " KB of "
       << size / 1024 << " KB, "
       << bytes * 10 / (1024 * (client->httpio->ds - starttime) + 1) << " KB/s"
       << endl;
}
...
int DemoApp::transfer_error(int td, int httpcode, int count) {
  cout << "TD " << td << ": Failed, HTTP error code " << httpcode << " (count "
       << count << ")" << endl;
...
void DemoApp::transfer_failed(int td, string &filename, error e) {
  if (e)
    cout << "TD " << td << ": Download failed (" << errorstring(e) << ")"
         << endl;
...
void DemoApp::transfer_failed(int td, error e) {
  if (e)
    cout << "TD " << td << ": Upload failed (" << errorstring(e) << ")" << endl;
}
...
void DemoApp::transfer_complete(int td, chunkmac_map *macs, const char *fn) {
  ...
  if (rename_file(tmpfilename.c_str(), filename.c_str()))
    cout << "TD " << td << ": Download complete: " << filename << endl;
  else
    cout << "TD " << td << ": rename(" << tmpfilename << "," << filename
         << ") failed" << endl;

  delete client->gettransfer((transfer_list *)&client->getq, td);
}

void DemoApp::transfer_complete(int td, handle ulhandle, const byte *ultoken,
                                const byte *filekey, SymmCipher *key) {
  cout << "TD " << td << ": Upload complete" << endl;

  FilePut *putf =
      (FilePut *)client->gettransfer((transfer_list *)&client->putq, td);
  ...
  if (putf) {
    ...
    if (putf->targetuser.size()) {
      cout << "Attempting to drop file into user " << putf->targetuser
           << "'s inbox..." << endl;
    ...
    cout << "(Canceled transfer, ignoring)" << endl;
...
void DemoApp::share_result(error e) {
  if (e)
    cout << "Share creation/modification request failed (" << errorstring(e)
         << ")" << endl;
}

void DemoApp::share_result(int, error e) {
  if (e)
    cout << "Share creation/modification failed (" << errorstring(e) << ")"
         << endl;
  else
    cout << "Share creation/modification succeeded" << endl;
}

void DemoApp::fa_complete(Node *n, fatype type, const char *data,
                          uint32_t len) {
  cout << "Got attribute of type " << type << " (" << len << " bytes) for "
       << n->displayname() << endl;
}

int DemoApp::fa_failed(handle, fatype type, int retries) {
  cout << "File attribute retrieval of type " << type
       << " failed (retries: " << retries << ")" << endl;
...
void DemoApp::putfa_result(handle, fatype, error e) {
  if (e)
    cout << "File attribute attachment failed (" << errorstring(e) << ")"
         << endl;
}
...
void DemoApp::notify_retry(dstime dsdelta) {
  cout << "API request failed, retrying in " << dsdelta * 100
       << " ms - Use 'retry' to retry immediately..." << endl;
}
...
static void listtrees() {
  for (int i = 0;
       i < (int)(sizeof client->rootnodes / sizeof *client->rootnodes); i++)
...
static Node *nodebypath(const char *ptr, string *user = NULL,
...
static void listnodeshares(Node *n) {
  for (share_map::iterator it = n->outshares.begin(); it != n->outshares.end();
       it++) {
    cout << "\t" << n->displayname();

    if (it->first)
      cout << ", shared with " << it->second->user->email << " ("
           << accesslevels[it->second->access] << ")" << endl;
    else
      cout << ", shared as exported folder link" << endl;
  }
}

void TreeProcListOutShares::proc(MegaClient *, Node *n) { listnodeshares(n); }

static void dumptree(Node *n, int recurse, int depth = 0,
  ...
  if (depth) {
    ...
    case FOLDERNODE:
      ...
      for (share_map::iterator it = n->outshares.begin();
           it != n->outshares.end(); it++) {
        if (it->first)
...
static const char *prompts[] = {"MEGA> ", "Password:", "Old Password:",
...
static const char *get_prompt(void) { return prompts[prompt]; }
...
void TreeProcCopy::allocnodes() { nn = new NewNode[nc]; }

TreeProcCopy::~TreeProcCopy() { delete[] nn; }
...
void process_line(char *line) {
  ...
  case LOGINPASSWORD:
    ...
    if (signupcode.size()) {
      ...
      if (*(uint64_t *)(signuppwchallenge + 4))
        ...
        client->confirmsignuplink(
            (const byte *)signupcode.data(), signupcode.size(),
  ...
  case PASSWORDCONFIRM:
    ...
    if (memcmp(pwkeybuf, newpwkey, sizeof pwkey))
      cout << endl << "Mismatch, please try again";
    else {
      error e;

      if (signupemail.size())
        client->sendsignuplink(signupemail.c_str(), signupname.c_str(),
                               newpwkey);
      else {
  ...
  case COMMAND:
    if (!line || !strcmp(line, "q") || !strcmp(line, "quit") ||
        !strcmp(line, "exit")) {
      term_restore();
      exit(0);
    }
    ...
    if (words[0] == "?" || words[0] == "h" || words[0] == "help") {
      ...
      cout << "      putua attrname [del|set string|load file] [private]"
           << endl;
    ...
    case 2:
      if (words[0] == "ls") {
        ...
        if (words.size() > 1) {
          if ((n = nodebypath(words[1].c_str()))) {
            if (client->checkaccess(n, FULL)) {
              ...
              if (e)
                cout << words[1] << ": Deletion failed (" << errorstring(e)
                     << ")" << endl;
            } else
              ...
              if (newname.size()) {
                if (tn->type == FILENODE) {
                  ...
                  if ((e = client->checkmove(n, tn)) == API_OK) {
                    ...
                    if ((e = client->setattr(n)))
                      cout << "Cannot rename file (" << errorstring(e) << ")"
                           << endl;
                  }
                }
              } else {
                if (tn->type == FILENODE) {
                  ...
                  if ((e = client->checkmove(n, tn->parent)) == API_OK) {
                    ...
                    if (e)
                      cout << "Rename failed (" << errorstring(e) << ")"
                           << endl;
                    ...
                    if (e)
                      cout << "Remove failed (" << errorstring(e) << ")"
                           << endl;
                  }
        ...
        if (words.size() > 2) {
          if ((n = nodebypath(words[1].c_str()))) {
            ...
            if (tn) {
              ...
              if (targetuser.size()) {
                cout << "Attempting to drop into user " << targetuser
                     << "'s inbox..." << endl;
        ...
        if (n) {
          ...
          cout << "Total storage used: " << (du.numbytes / 1048576) << " MB"
               << endl;
    ...
    case 3:
      if (words[0] == "get") {
          ...
          if (n) {
            ...
            if (n->type == FILENODE)
              ...
              for (node_list::iterator it = n->children.begin();
                ...
                if ((*it)->type == FILENODE)
                  client->getq.push_back(new AppFileGet((*it)->nodehandle));
            }
          } else
            cout << words[1] << ": No such file or folder" << endl;
        } else
          cout << "      get remotefile/remotepath" << endl;
        ...
        if (words.size() > 1) {
          ...
          int total = globenqueue(words[1].c_str(), newname.c_str(), target,
                                  targetuser.c_str());

          cout << "Queued " << total << " file(s) for upload, "
               << client->putq.size() << " file(s) in queue" << endl;
        } else
          cout << "      put localpattern [dstpath|dstemail:]" << endl;
        ...
        if (words.size() > 1) {
          if (!change_dir(words[1].c_str()))
            cout << words[1].c_str() << ": Failed (" << errno << ")" << endl;
        } else
          cout << "      lcd [localpath]" << endl;
    ...
    case 4:
      if (words[0] == "putq") {
        ...
        if (words.size() > 1)
          cancel = atoi(words[1].c_str());
        else
          cancel = -1;

        for (put_list::iterator it = client->putq.begin();
             it != client->putq.end(); s++) {
          if (cancel < 0 || s == cancel) {
            cout << s << ": " << (*it)->filename << " -> ";

            if ((*it)->targetuser.size())
              cout << (*it)->targetuser << ":";
            else {
              string path;
        ...
        for (get_list::iterator it = client->getq.begin();
             it != client->getq.end(); s++) {
    ...
    case 5:
      if (words[0] == "login") {
          ...
          if (words.size() > 1) {
            if (strchr(words[1].c_str(), '@')) {
              ...
              if ((ptr = strstr(words[1].c_str(), "#F!")) && ptr[11] == '!') {
                client->app->login_result(
                    client->folderaccess(ptr + 3, ptr + 12));
              } else {
                cout << "Invalid argument. Please specify a valid e-mail "
                        "address or a folder link containing the folder key."
                     << endl;
              }
            }
          } else
        ...
        if (words.size() == 1) {
          ...
          if (Base64::atob(words[1].c_str(), (byte *)&uh, sizeof uh) -
                      (byte *)&uh ==
                  sizeof uh &&
              Base64::atob(words[1].c_str() + 12, pw, sizeof pw) - pw ==
                  sizeof pw)
        ...
        case 1: // list all shares (incoming and outgoing)
        {
          ...
          for (unsigned i = 0;
               i < sizeof client->rootnodes / sizeof *client->rootnodes; i++)
        ...
        if (words.size() > 1) {
          ...
          if (n) {
            ...
            cout << "Fetching " << c << " file attribute(s) of type " << type
                 << "..." << endl;
          }
        } else
          cout << "      getfa type [path]" << endl;
        ...
        if (words.size() == 2) {
          ...
          if (words[2] == "del") {
            ...
            client->putua(words[1].c_str(), (const byte *)words[3].c_str(),
                          words[3].size(), priv);
            ...
            if (loadfile(words[3].c_str(), &data))
              client->putua(words[1].c_str(), (const byte *)data.data(),
                            data.size(), priv);
            else
    ...
    case 6:
      if (words[0] == "passwd") {
        ...
        if (words.size() == 2) {
          ...
          unsigned len =
              (words[1].size() - (ptr - words[1].c_str())) * 3 / 4 + 4;

          byte *c = new byte[len];
          len = Base64::atob(ptr, c, len) - c;
          client->querysignuplink(
              c,
              len); // we first just query the supplied signup link, then
                    // collect and verify the password, then confirm the account
          delete[] c;
        } else if (words.size() == 3) {
          ...
          case NOTLOGGEDIN:
            cout << "Please use the begin command to commence or resume the "
                    "ephemeral session to be upgraded."
                 << endl;
          }
        }
        ...
        if (words.size() > 1) {
          ...
          if ((n = nodebypath(words[1].c_str()))) {
            ...
            if ((e = client->exportnode(n,
                                        words.size() > 2 && words[2] == "del")))
              cout << words[1] << ": Export rejected (" << errorstring(e) << ")"
                   << endl;
            else
              cout << "Exporting..." << endl;
          } else
        ...
        if (words.size() > 1) {
          if (client->openfilelink(words[1].c_str()) == API_OK)
            cout << "Opening link..." << endl;
          else
            cout << "Malformed link. Format: Exported URL or fileid#filekey"
                 << endl;
        } else
          cout << "      import exportedfilelink#key" << endl;
    ...
    case 7:
      if (words[0] == "confirm") {
        if (signupemail.size() && signupcode.size()) {
          cout << "Please type " << signupemail
               << "'s password to confirm the signup." << endl;
          setprompt(LOGINPASSWORD);
        } else
...
// this can occur e.g. with syntactically malformed requests (due to a bug), an
...
void DemoApp::sendsignuplink_result(error e) {
  if (e)
    ...
    cout << "Thank you. Please check your e-mail and enter the command signup "
            "followed by the confirmation link."
         << endl;
}

// signup link query result
void DemoApp::querysignuplink_result(handle uh, const char *email,
                                     const char *name, const byte *pwc,
                                     const byte *kc, const byte *c,
                                     size_t len) {
  cout << "Ready to confirm user account " << email << " (" << name
       << ") - enter confirm to execute." << endl;
...
void DemoApp::exportnode_result(handle h, handle ph) {
  ...
  if ((n = client->nodebyhandle(h))) {
    ...
    // the key
    if (n->type == FILENODE)
      Base64::btoa((const byte *)n->nodekey.data(), Node::FILENODEKEYLENGTH,
                   key);
    else if (n->sharekey)
    ...
    cout << "https://mega.co.nz/#" << (n->type ? "F" : "") << "!" << node << "!"
         << key << endl;
  } else
...
void DemoApp::reload(const char *reason) {
  cout << "Reload suggested (" << reason << ") - use 'reload' to trigger"
       << endl;
}
...
// (nodes with their removed flag set will be deleted immediately after
// returning from this call, at which point their pointers will become invalid
// at that point.)
void DemoApp::nodes_updated(Node **n, int count) {
  ...
  if (n) {
    ...
    for (node_map::iterator it = client->nodes.begin();
         it != client->nodes.end(); it++)
      if (it->second->type < 6)
        c[1][it->second->type]++;
  }
...
void DemoApp::account_details(AccountDetails *ad, int storage, int transfer,
                              int pro, int purchases, int transactions,
                              int sessions) {
  ...
  if (transfer) {
    if (ad->transfer_max) {
      cout << "\tTransfer in progress: " << ad->transfer_own_reserved << "/"
           << ad->transfer_srv_reserved << endl;
      cout << "\tTransfer completed: " << ad->transfer_own_used << "/"
           << ad->transfer_srv_used << " of " << ad->transfer_max << " ("
  ...
  if (pro) {
    ...
    for (vector<AccountBalance>::iterator it = ad->balances.begin();
         it != ad->balances.end(); it++) {
  ...
  if (purchases) {
    ...
    for (vector<AccountPurchase>::iterator it = ad->purchases.begin();
         it != ad->purchases.end(); it++) {
  ...
  if (transactions) {
    ...
    for (vector<AccountTransaction>::iterator it = ad->transactions.begin();
         it != ad->transactions.end(); it++) {
      strftime(timebuf, sizeof timebuf, "%c", localtime(&it->timestamp));
      printf("\tID: %.11s Time: %s Delta: %.3s %.02f\n", it->handle, timebuf,
             it->currency, it->delta);
    }
  }
  ...
  if (sessions) {
    ...
    for (vector<AccountSession>::iterator it = ad->sessions.begin();
         it != ad->sessions.end(); it++) {
      strftime(timebuf, sizeof timebuf, "%c", localtime(&it->timestamp));
      strftime(timebuf2, sizeof timebuf, "%c", localtime(&it->mru));
      printf("\tSession start: %s Most recent activity: %s IP: %s Country: "
             "%.2s User-Agent: %s\n",
             timebuf, timebuf2, it->ip.c_str(), it->country,
             it->useragent.c_str());
    }
  }
}
...
void DemoApp::account_details(AccountDetails *ad, error e) {
  if (e)
    cout << "Account details retrieval failed (" << errorstring(e) << ")"
         << endl;
}

// user attribute update notification
void DemoApp::userattr_update(User *u, int priv, const char *n) {
  cout << "Notification: User " << u->email << " -" << (priv ? " private" : "")
       << " attribute " << n << " added or updated" << endl;
}
/*
// main loop
void megacli()
